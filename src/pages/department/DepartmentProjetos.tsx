...
import { useProjetosDept } from "@/hooks/useDepartamento";
...
export default function DepartmentProjetos() {
  ...
  const { data: projetos = d.projetos } = useProjetosDept(key);

  return (
    <section>
      <div className="mb-[22px]">
        <div className="text-[21px] font-semibold">{d.nome} — Projetos</div>
        <div className="mt-1 text-[13px] text-muted">{projetos.length} projeto(s) neste departamento.</div>
      </div>

      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {projetos.map((p) => (
          <Card key={p.nome} className="p-[18px] pb-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-[11px] text-muted">Prazo {p.prazo}</span>
            </div>
          </Card>
        ))}
      </div>
